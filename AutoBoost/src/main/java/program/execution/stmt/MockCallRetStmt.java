...
public class MockCallRetStmt extends Stmt {
    ...
    public String getStmt(Set<Class<?>> fullCNameNeeded) {
        if (skipChecking)
            return "PowerMockito.doReturn(" + returnStmts.stream().map(s -> s.getStmt(fullCNameNeeded)).collect(Collectors.joining(",")) + ").when(" + (methodInvStmt.getCallee() + ")." + methodInvStmt.getInstanceCallString(fullCNameNeeded));
