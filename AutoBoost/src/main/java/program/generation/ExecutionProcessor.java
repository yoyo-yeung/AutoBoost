...
public class ExecutionProcessor {
    ...
    protected boolean testSetUp(MethodExecution target) {
        return isPossibleTarget(target) && targetIsBestMatch(target) && canConstructCallee(target) && checkAndSetRequiredPackage(target)  && canRecreateParams(target) && (target.getRequiredPackage().isEmpty() || target.getRequiredPackage().startsWith(Properties.getSingleton().getPUT()));
    }
    ...
    private boolean canRecreateParams(MethodExecution target) {
        ...
        return IntStream.range(0, target.getParams().size())
                .allMatch(pID -> {
                    ...
                    Class<?> paramType = sootTypeToClass(target.getMethodInvoked().getParameterTypes().get(pID));
...
//                        })) return true;
    ...
    private boolean canProvideReturnVal(MethodExecution execution, VarDetail returnVal) {
        ...
        if (returnVal instanceof ObjVarDetails && !returnVal.equals(executionTrace.getNullVar()) && ((declaredReturnType == null && getRequiredPackage(returnVal.getType()) == null) || (declaredReturnType != null && !declaredReturnType.isAssignableFrom(getAccessibleSuperType(returnVal.getType(), execution.getRequiredPackage())))))
            return false;
    ...
    private boolean canRecreateParam(MethodExecution target, Class<?> paramType, VarDetail p) {
        if (p instanceof ObjVarDetails) {
            ...
            if (paramType != null && !paramType.isAssignableFrom(getAccessibleMockableSuperType(p.getType(), target.getRequiredPackage())))
                return false;
            if (paramType == null && getRequiredPackage(p.getType()) == null) return false;
            if (executionTrace.hasFieldAccess(target, p) && p.getType().getPackage().getName().startsWith(Properties.getSingleton().getPUT()))
                return false;
        ...
        if (p instanceof EnumVarDetails) {
            ...
            try {
                if (!p.getType().equals(Class.class) && !p.getType().isEnum()) {
                    if (p.getType().getPackage().getName().startsWith(Properties.getSingleton().getPUT())) {
                ...
                if (p.getType().equals(Class.class))
                    requiredPackage = getRequiredPackage(ClassUtils.getClass(((EnumVarDetails) p).getValue()));
                if (p.getType().isEnum())
                    requiredPackage = getRequiredPackage(p.getType());
    ...
    private Set<Integer> getUnmockableInputs(MethodExecution execution) {
        return execution.getParams().stream()
                ...
                .filter(p -> Helper.isCannotMockType(p.getType()) && (executionTrace.getParentExeStack(p, true) == null || executionTrace.getParentExeStack(p, true).stream().anyMatch(e -> e.getMethodInvoked().getDeclaringClass().getPackageName().startsWith(Properties.getSingleton().getPUT())))) // do not track the var if they are NOT to be mocked
                .map(VarDetail::getID)
                .collect(Collectors.toSet());
    }
    ...
    private boolean canConstructCallee(MethodExecution target) {
        ...
        if (callee instanceof EnumVarDetails) {
            if (!callee.getType().isEnum()) {
                try {
                    if (!callee.getType().getPackage().getName().startsWith(Properties.getSingleton().getPUT()) && !soot.Modifier.isPublic(callee.getType().getField(((EnumVarDetails) callee).getValue()).getModifiers()))
                        return false;
                } catch (NoSuchFieldException e) {
                    return false;
                }
                return !callee.getType().getPackage().getName().startsWith(Properties.getSingleton().getPUT()) || InstrumentResult.getSingleton().getClassPublicFieldsMap().getOrDefault(callee.getType().getName(), new HashSet<>()).contains(((EnumVarDetails) callee).getValue());
            }
        }
        return true;
    }
    ...
    private boolean checkAndSetRequiredPackage(MethodExecution target) {
        ...
        if (target.getCalleeId() != -1) {
            ...
            if (callee instanceof EnumVarDetails) {
                if (!callee.getType().isEnum()) {
                    requiredPackage = getRequiredPackage(callee.getType());
                }
            }
        } else if (target.getMethodInvoked().getType().equals(METHOD_TYPE.STATIC)) {
            requiredPackage = getRequiredPackage(target.getMethodInvoked().getdClass());
        }
    ...
    private boolean isPossibleTarget(MethodExecution execution) {
        ...
        if (execution.getCalleeId() != -1) {
            ...
            if (callee.getType().getPackage() == null || !callee.getType().getPackage().getName().startsWith(Properties.getSingleton().getPUT()))
                return false;
        }
    ...
    private boolean varDetailIsAssertable(MethodExecution target, VarDetail returnVal) {
        ...
        if (returnVal instanceof EnumVarDetails) {
            try {
                if (returnVal.getType().equals(Class.class) && !accessibilityCheck(ClassUtils.getClass(((EnumVarDetails) returnVal).getValue()), target.getRequiredPackage()))
                    return false;
                if (returnVal.getType().isEnum())
                    return accessibilityCheck(returnVal.getType(), target.getRequiredPackage());
...
//            if (creatingClass.getName().startsWith(Properties.getSingleton().getPUT()))
    ...
    private Object getRecreatedParam(VarDetail varDetail) {
        ...
        if (varDetail instanceof PrimitiveVarDetails) return ((PrimitiveVarDetails) varDetail).getValue();
        ...
        if (varDetail instanceof WrapperVarDetails) return ((WrapperVarDetails) varDetail).getValue();
