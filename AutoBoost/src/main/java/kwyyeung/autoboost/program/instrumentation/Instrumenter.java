package kwyyeung.autoboost.program.instrumentation;
...
import static kwyyeung.autoboost.helper.Helper.isCannotMockType;
...
public class Instrumenter extends BodyTransformer {
    ...
    private static final kwyyeung.autoboost.helper.Properties properties = Properties.getSingleton();
    ...
    static {
        loggerClass = Scene.v().loadClassAndSupport("kwyyeung.autoboost.program.execution.ExecutionLogger");
        ...
        logFieldAccessMethod = loggerClass.getMethod("void logFieldAccess(int,java.lang.Object,long)");
    ...
    private void processModifiers(SootClass currentDeclaringClass, SootMethod currentSootMethod) {
    ...
    protected void internalTransform(Body body, String phaseName, Map<String, String> options) {
        ...
        processModifiers(currentSootMethod.getDeclaringClass(), currentSootMethod);
        ...
        while (stmtIt.hasNext()) {
            ...
            if (stmt instanceof JIdentityStmt) {
                ...
                if(!body.getMethod().isStatic() && ((JIdentityStmt) stmt).getRightOp() instanceof InstanceFieldRef && ((InstanceFieldRef) ((JIdentityStmt) stmt).getRightOp()).getBase().equals(body.getThisLocal()))
                    methodDetails.getAccessedCalleeFields().add(new ImmutablePair<>(((InstanceFieldRef) ((JIdentityStmt) stmt).getRightOp()).getField().getDeclaringClass().getName(), ((InstanceFieldRef) ((JIdentityStmt) stmt).getRightOp()).getField().getName()));
                continue;
            }
            // set thread id
            if (!threadRetrieved) {
...
//
            ...
            if(!body.getMethod().isStatic() && stmt instanceof DefinitionStmt && ((DefinitionStmt) stmt).getRightOp() instanceof InstanceFieldRef && ((InstanceFieldRef) ((DefinitionStmt) stmt).getRightOp()).getBase().equals(body.getThisLocal())) {
                methodDetails.getAccessedCalleeFields().add(new ImmutablePair<>(((InstanceFieldRef) ((DefinitionStmt) stmt).getRightOp()).getField().getDeclaringClass().getName(), ((InstanceFieldRef) ((DefinitionStmt) stmt).getRightOp()).getField().getName()));
            }
...
//        List<Unit> toInsert = new ArrayList<>(getReturnLocalSetupStmt(localGenerator, fieldRef.getType(), returnVal, originalReturnVal));
        List<Unit> toInsert = new ArrayList<>();
        Expr logExpr = Jimple.v().newStaticInvokeExpr(logFieldAccessMethod.makeRef(), IntConstant.v(methodDetails.getId()), base, threadIDLocal);
