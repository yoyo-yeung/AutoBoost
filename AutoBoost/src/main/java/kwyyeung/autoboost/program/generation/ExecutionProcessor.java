...
public class ExecutionProcessor {
    ...
    private Map executionToTrackValMap = new HashMap<>();

    protected boolean testSetUp(MethodExecution target) {
        return isPossibleTarget(target) && targetIsBestMatch(target) && canConstructCallee(target) && checkAndSetRequiredPackage(target) && canRecreateTargetParams(target) && (target.getRequiredPackage().isEmpty() || target.getRequiredPackage().startsWith(kwyyeung.autoboost.helper.Properties.getSingleton().getPUT()));
    }
    ...
    protected boolean canRecreateTargetParams(MethodExecution target) {
        Map<Integer, Integer> mocks = new HashMap<>();
        Set<Integer> cannotMocks = new HashSet<>();
        // theoritically, the fields of callee would be set directly, params used for constructing would not persist
        Map<Boolean, Set<VarDetail>> mocking = getMockableInputs(target);
        mocks = mocking.getOrDefault(true, new HashSet<>()).stream().map(VarDetail::getID).collect(Collectors.toMap(i -> i, i -> i));
        cannotMocks = mocking.getOrDefault(false, new HashSet<>()).stream().map(VarDetail::getID).collect(Collectors.toSet());
        boolean res = IntStream.range(0, target.getParams().size())
                .allMatch(pID -> canRecreateParam(target, Helper.sootTypeToClass(target.getMethodInvoked().getParameterTypes().get(pID)), executionTrace.getVarDetailByID(target.getParams().get(pID)), new HashSet<>())) && !hasUnmockableUsages(target, mocks, cannotMocks, new HashSet<>(), target.getMockOccurrences());
        target.setVarToMock(mocks.values().stream().collect(Collectors.toSet()));
        return res;
    }
    ...
    private boolean hasUnmockableUsages(MethodExecution target, Map<Integer, Integer> mockables, Set<Integer> unmockables, Set<MethodExecution> processed, Set<MockOccurrence> occurrences) {
        if (processed.contains(target)) return false;
        processed.add(target);
        if (executionTrace.getChildren(target.getID()).stream()
                .map(executionTrace::getMethodExecutionByID)
                .anyMatch(e -> {
                    MethodDetails md = e.getMethodInvoked();
                    if (unmockables.contains(e.getCalleeId())) return true;
                    if (mockables.containsKey(e.getCalleeId())) {
                        if (md.isFieldAccess()) return true;
                        if ((md.getName().equals("equals") || md.getName().equals("hashCode") || md.getName().equals("getClass")))
                            return true;
                        if (e.getReturnValId() != -1 && !canProvideReturnVal(e, executionTrace.getVarDetailByID(e.getReturnValId())))
                        ...
                        if(e.getReturnValId() != -1 ) {
                            MockOccurrence occurrence = occurrences.stream().filter(o -> o.sameCallInfo(mockables.get(e.getCalleeId()), e)).findAny().orElse(null);
                            if(occurrence == null) {
                                occurrence = new MockOccurrence(mockables.get(e.getCalleeId()), e.getMethodInvoked(), e.getParams());
                                occurrences.add(occurrence);
                            }
                            occurrence.addReturnVar(e.getReturnValId());
                            occurrence.getMockingExes().add(e);
                            if(TestGenerator.isVarToMock(executionTrace.getVarDetailByID(e.getReturnValId())))
                                mockables.put(e.getReturnValId(), e.getReturnValId());
                        }
                        mockables.put(e.getResultThisId(), mockables.get(e.getCalleeId()));
                        return false;
                    }
                    if (InstrumentResult.getSingleton().isLibMethod(md.getId()) && e.getParams().stream().anyMatch(mockables::containsKey))
                        return true;

                    return hasUnmockableUsages(e, mockables, unmockables, processed, occurrences);
                })) {
            return true;
        }
        processed.remove(target);
...
//                logger.error(((EnumVarDetails) p).getValue() + "  not found ");
    ...
    private Map<Boolean, Set<VarDetail>> getMockableInputs(MethodExecution execution) {
        Map<Boolean, Set<VarDetail>> res = execution.getParams().stream()
                .map(executionTrace::getVarDetailByID)
                .map(executionTrace::getRelatedObjVarIDs)
                .flatMap(Collection::stream)
                .map(executionTrace::getVarDetailByID)
                .collect(Collectors.groupingBy(p -> !Helper.isCannotMockType(p.getType()), Collectors.toSet()));
        if(res.containsKey(false))
            res.get(false).removeIf(p -> !(executionTrace.getParentExeStack(p, true) == null || executionTrace.getParentExeStack(p, true).stream().anyMatch(e -> e.getMethodInvoked().getDeclaringClass().getPackageName().startsWith(kwyyeung.autoboost.helper.Properties.getSingleton().getPUT()))));
        return res;
    }
...
//            if (creatingClass.getName().startsWith(Properties.getSingleton().getPUT()))
            ...
            if (cache) {
                if(defExe != null) defExe.setDefExe(true);
...
//                    logger.error(((EnumVarDetails) varDetail).getValue() + " field not found ");
        ...
        if(!Helper.isCannotMockType(varDetail.getType()) && !Modifier.isFinal(varDetail.getType().getModifiers())){
            return Mockito.mock(varDetail.getType());
        }
        return null;
    }
...
//        if (!this.classToDefExeMap.containsKey(paramType) && !processing.contains(paramType)) getExeConstructingClass(paramType, processing, true);
    ...
    private boolean canReenact(MethodExecution methodExecution) {
        ...
        return methodExecution.getParams().stream()
                ...
                allMatch(v -> !Helper.isCannotMockType(v.getType()) && !Modifier.isFinal(v.getType().getModifiers())
                );
    }
    ...
    void findAllThisFields(MethodExecution target) {
        findAllThisFields(target, new HashSet<>());
    }
    private void findAllThisFields(MethodExecution target, Set<MethodExecution> detecting) {
        if(!target.getMethodInvoked().getType().equals(METHOD_TYPE.MEMBER)) return; // if there is no 'callee' for the execution, no need to find fields of callee accessed
        if(target.getAccessedCalleeFields() != null) return; // if such fields were acquired already
        if(detecting.contains(target)) return;
        detecting.add(target);
        List<MethodExecution> childExes = executionTrace.getChildren(target.getID()).stream()
                .map(executionTrace::getMethodExecutionByID)
                .filter(c -> c.getMethodInvoked().getType().equals(METHOD_TYPE.MEMBER))
                .collect(Collectors.toList());
        List<MethodExecution> reversedExes = new ArrayList<>(childExes);
        Collections.reverse(reversedExes);
        Set<Integer> possibleCalleeIds = new HashSet<Integer>(){{add(target.getCalleeId()); add(target.getResultThisId());}};

        /**
         * Find all possible instance VarDetailID by scanning in order + in reverse order
         * Reason for 2 scans:
         * Some method executions in the middle (before / after direct this.field assignment) cannot be detected if only one scan is used when their callee id does not match the resultThisId of previous execution / resultThisId does not match the callee ID of the subsequent execution (due to direct field value modification between executions)
         * Limitation:
         * If the method execution is between field modifications, it cannot be detected
         */
        Stream.concat(childExes.stream(), reversedExes.stream())
                .filter(c -> possibleCalleeIds.contains(c.getCalleeId()) || possibleCalleeIds.contains(c.getResultThisId())).forEach(c -> {
            possibleCalleeIds.add(c.getCalleeId());
            possibleCalleeIds.add(c.getResultThisId());
        });

        Set<Pair> accessedFields = childExes.stream()
                .filter(c -> possibleCalleeIds.contains(c.getCalleeId()))
                .map(c -> {findAllThisFields(c, detecting); return c.getAccessedCalleeFields();})
                .filter(Objects::nonNull)
                .flatMap(cp -> cp.stream())
                .collect(Collectors.toSet());
        accessedFields.addAll(target.getMethodInvoked().getAccessedCalleeFields());
        target.setAccessedCalleeFields(accessedFields);
        detecting.remove(target);
    }

    protected void prepareExeTrackValue(MethodExecution target) {
        if (this.executionToTrackValMap.containsKey(target)) return;
        // callee details, methodID, param details (id if concrete, else mocking details), return val
        List trackVal = new ArrayList(4);
        if (target.getCallee() != null) {
            if (target.getCallee() instanceof ObjVarDetails)
                trackVal.add(XMLParser.fromXMLtoContentMap(target.getCallee(), (String) target.getCallee().getValue()).entrySet().stream()
                        .filter(e -> target.getAccessedCalleeFields().contains(e.getKey()))
                        .collect(Collectors.toSet()) );
            else trackVal.add(target.getCalleeId());
        }
        else trackVal.add(null);
        trackVal.add(target.getMethodInvoked().getId());
        trackVal.add(target.getParams().stream().map(executionTrace::getVarDetailByID)
                .map(p -> mockTrackValue(p, target.getVarToMock(), target.getMockOccurrences(), new HashSet())).collect(Collectors.toList()));
        trackVal.add(target.getReturnValId());
        this.executionToTrackValMap.put(target, trackVal);

    }
    private Object mockTrackValue(VarDetail p, Set<Integer> mocks, Set<MockOccurrence> mockOccurrences, Set tracking) {
        if(tracking.contains(p.getID())) return -1;
        Object res = p.getID();
        tracking.add(p.getID());
        if(p instanceof ArrVarDetails) {
            if(!p.getType().isArray() || !p.getType().getComponentType().isPrimitive())
                res = ((ArrVarDetails) p).getComponents().stream().map(executionTrace::getVarDetailByID).map(c -> this.mockTrackValue(c,mocks,  mockOccurrences, tracking)).collect(Collectors.toList());
        }
        else if(p instanceof MapVarDetails) {
            res = ((MapVarDetails) p).getKeyValuePairs().stream().map(pke -> new ImmutablePair(this.mockTrackValue(executionTrace.getVarDetailByID(pke.getKey()),mocks, mockOccurrences,  tracking), this.mockTrackValue(executionTrace.getVarDetailByID(pke.getValue()),mocks, mockOccurrences, tracking))).collect(Collectors.toSet());
        }
        else if(p instanceof ObjVarDetails) {
            res = !mocks.contains(p.getID()) ? p.getID() : mockOccurrences.stream().filter(mo -> mo.getCallee() == p.getID()).map(mo ->  Arrays.asList(mo.getInovkedMethod().getId(), mo.getParamVarID().stream().map(pi -> TestGenerator.isVarToMock(executionTrace.getVarDetailByID(pi))? -1 : pi).collect(Collectors.toList()), mo.getReturnVars().stream().map(rv -> mockTrackValue(executionTrace.getVarDetailByID(rv), mocks, mockOccurrences, tracking)).collect(Collectors.toList())))
                    .collect(Collectors.toSet());
        }
        tracking.remove(p.getID());
        return res;
    }

    public Map getExecutionToTrackValMap() {
        return executionToTrackValMap;
    }
}
