...
/**
 * This class handles processing of Method Executions during test runtime
 */
...
//            checkVal = getComponentStream(execution, type, objValue, process, canOnlyBeUse, hashProcessing, depth - 1, processedHashToVarIDMap).collect(Collectors.toList());
    ...
    public VarDetail getVarDetail(MethodExecution execution, IntermediateVarContent varContent, int hashCode, LOG_ITEM process, boolean canOnlyBeUse, Map<Integer, Integer> processedHashToVarIDMap) {
        ...
        if (varDetail instanceof ObjVarDetails && !varDetail.getType().getName().startsWith(kwyyeung.autoboost.helper.Properties.getSingleton().getPUT()) && !canOnlyBeUse) {
    ...
    private boolean setCurrentExeAsDef(VarDetail varDetail, LOG_ITEM process, MethodExecution execution) {
        ...
        if (methodDetails.getType().equals(METHOD_TYPE.MEMBER) && (getParentExeStack(execution.getCallee(), false) == null || getParentExeStack(execution.getCallee(), false).contains(execution)))
            return false;
        if (Helper.isCannotMockType(varDetail.getType()) && methodDetails.getDeclaringClass().getPackageName().startsWith(kwyyeung.autoboost.helper.Properties.getSingleton().getPUT()))
            return false; // would not use as def if it is an un-mock-able param type created by PUT methods
        if (kwyyeung.autoboost.helper.Properties.getSingleton().getFaultyFuncIds().contains(execution.getMethodInvoked().getId()) || containsFaultyDef(execution, true))
            return false;
...
//            objValue = ((Set<Map.Entry>) objValue).stream().map(e -> e.getKey() + "=" + e.getValue()).sorted().collect(Collectors.joining(Properties.getDELIMITER()));
    ...
    public Set<ObjVarDetails> getAllObjVarInvolved(VarDetail varDetail) {
        ...
        if (varDetail.equals(getNullVar())) return res;
    ...
    public Set<Integer> getRelatedObjVarIDs(VarDetail varDetail, Set<Integer> relatedVarIDs) {
        ...
        if (varDetail instanceof StringVarDetails || varDetail instanceof StringBVarDetails || varDetail instanceof PrimitiveVarDetails || varDetail instanceof EnumVarDetails || varDetail instanceof WrapperVarDetails || varDetail.equals(ExecutionTrace.getSingleton().getNullVar()))
