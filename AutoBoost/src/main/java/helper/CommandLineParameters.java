...
public class CommandLineParameters {
    ...
    public static void handleTestRunnerClass(CommandLine line) {
        ...
        if (!line.hasOption("testRunnerClass"))
            properties.setTestRunnerClass(Properties.DEFAULT_TEST_RUNNER_CLASS);
        else
            properties.setTestRunnerClass(line.getOptionValue("testRunnerClass"));
    }

    public static void handleDependencyPaths(CommandLine line) {
        Properties properties = Properties.getInstance();
        if (!line.hasOption("dependencyPaths"))
            properties.setDependencyPaths(new String[0]);
        else {
            ...
            properties.setDependencyPaths(dirPaths);
        }
    }
